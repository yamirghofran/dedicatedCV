...
export function MinimalTemplate({ cv }: Props) {
  return (
    <div style={{ ...base, background: '#fff', padding: 32, display: 'grid', gap: 12 }}>
          ...
          <ul style={{ paddingLeft: 16, margin: 0 }}>
            {cv.work_experiences.map((w) => (
              <li key={w.id} style={{ marginBottom: 6 }}>
                <div style={{ ...sm, fontWeight: 600 }}>{w.position} — {w.company}</div>
                <div style={xs}>
          ...
          <ul style={{ paddingLeft: 16, margin: 0 }}>
            {cv.educations.map((e) => (
              <li key={e.id} style={{ marginBottom: 6 }}>
                <div style={{ ...sm, fontWeight: 600 }}>{e.degree} — {e.institution}</div>
                <div style={xs}>
          ...
          <ul style={{ paddingLeft: 16, margin: 0 }}>
            {cv.projects.map((p) => (
              <li key={p.id} style={{ marginBottom: 6 }}>
                <div style={{ ...sm, fontWeight: 600 }}>{p.name || (p as any).title}</div>
