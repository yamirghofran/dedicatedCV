...
export function ModernTemplate({ cv }: Props) {
  return (
    <div style={{ ...base, background: '#f8fafc', padding: 32, display: 'grid', gap: 16 }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 8 }}>
        ...
        {cv.location && (
          <span style={{ background: '#0f172a', color: '#fff', padding: '6px 10px', borderRadius: 999, fontSize: 12 }}>
            {cv.location}
          </span>
        )}
      </header>
        ...
        <section>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            {cv.work_experiences.map((w) => (
              <div key={w.id} style={{ border: '1px solid #e2e8f0', background: '#fff', borderRadius: 8, padding: 12 }}>
                ...
                {w.description && <p style={{ ...small, marginTop: 4 }}>{w.description}</p>}
              </div>
            ))}
          </div>
        </section>
        ...
        {cv.educations?.length > 0 && (
          <section>
            ...
            <div style={{ display: 'grid', gap: 8 }}>
              {cv.educations.map((e) => (
                <div key={e.id} style={{ border: '1px solid #e2e8f0', background: '#fff', borderRadius: 8, padding: 12 }}>
                  <div style={{ ...small, fontWeight: 600 }}>{e.institution}</div>
                  <div style={xs}>
        ...
        {cv.skills?.length > 0 && (
          <section>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {cv.skills.map((s) => (
                <span key={s.id} style={{ border: '1px solid #e2e8f0', background: '#fff', borderRadius: 999, padding: '4px 10px', fontSize: 12 }}>
                  {s.name}
                </span>
              ))}
            </div>
      ...
      {cv.projects?.length > 0 && (
        <section>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            {cv.projects.map((p) => (
              <div key={p.id} style={{ border: '1px solid #e2e8f0', background: '#fff', borderRadius: 8, padding: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, fontWeight: 600 }}>
                ...
                {p.description && <p style={{ ...small, marginTop: 4 }}>{p.description}</p>}
              </div>
            ))}
          </div>
        </section>
